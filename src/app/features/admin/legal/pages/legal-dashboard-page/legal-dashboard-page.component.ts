...
export class LegalDashboardPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParamMap.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((params) => {
      ...
      this.statusFilter.set(status === 'Published' || status === 'Draft' || status === 'Archived' ? status : null);
      this.footerFilter.set(null);
      this.navFilter.set(null);
      this.acceptanceFilter.set(null);
      void this.reload();
    });
  }
